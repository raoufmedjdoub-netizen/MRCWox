<?php

return array(

    'password' => 'يجب أن تتكون كلمات المرور من ستة أحرف على الأقل وأن تتطابق مع التأكيد.',
    'reset' => ' تم إعادة تعيين كلمة المرور الخاصة بك',
    'sent' => 'لقد أرسلنا رابط إعادة تعيين كلمة المرور بالبريد الإلكتروني!',
    'token' => 'رمز إعادة تعيين كلمة المرور هذا غير صالح.',
    'user' => 'لا يمكننا العثور على مستخدم بعنوان البريد الإلكتروني هذا.',
    'throttled' => 'الرجاء الانتظار قبل إعادة المحاولة.',
    'generic' => 'إذا كان هناك حساب يحتوي على هذه البيانات، فسوف تتلقى رابطًا لإعادة تعيين كلمة المرور.',
    'action_confirmation_with_password' => 'الرجاء إدخال كلمة المرور لتأكيد هذا الإجراء',
    'via_phone' => 'الاسترداد عن طريق رقم الهاتف',
    'via_email' => 'الاسترداد عبر البريد الإلكتروني',
    'recovery_via' => 'كيف ترغب في تلقي رابط إعادة تعيين كلمة المرور؟',
);