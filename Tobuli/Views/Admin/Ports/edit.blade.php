...
@section('body')
    ...
    <div class="form-group">
        {!! Form::label('name', trans('validation.attributes.child_ports') . ':') !!}

        @php $i = 0 @endphp
        @foreach($childPorts as $port => $name)
            <div class="row row-padding" style="padding-bottom:10px">
                <div class="col-xs-6">
                    {!! Form::text("children[$i][name]", $name, ['class' => 'form-control', 'placeholder' => trans('validation.attributes.name')]) !!}
                </div>
                <div class="col-xs-6">
                    <div class="input-group">
                        {!! Form::text("children[$i][port]", $port, ['class' => 'form-control', 'placeholder' => trans('validation.attributes.port')]) !!}
                        <span class="input-group-addon"><a href="javascript:" class="delete-children-item remove-icon"><span aria-hidden="true">×</span></a></span>
                    </div>
                </div>
            </div>
            @php $i++ @endphp
        ...
        <div class="row row-padding children-empty" style="padding-bottom:10px">
            <div class="col-xs-6">
                {!! Form::text("children[empty][name]", null, ['class' => 'form-control', 'placeholder' => trans('validation.attributes.name')]) !!}
            </div>
            <div class="col-xs-6">
                <div class="input-group">
                    {!! Form::text("children[empty][port]", null, ['class' => 'form-control', 'placeholder' => trans('validation.attributes.port')]) !!}
                    <span class="input-group-addon"><a href="javascript:" class="delete-children-item remove-icon"><span aria-hidden="true">×</span></a></span>
                </div>
            </div>
        </div>
