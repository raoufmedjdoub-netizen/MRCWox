...
class AppendFuelTheftByPeaks extends ActionAppend
{
    ...
    public function boot()
    {
        $this->sensors = $this->getDevice()->sensors->filter(fn ($sensor) => $sensor->type == 'fuel_tank')
            ->keyBy('id')
            ->all();
        $this->minFuelChange = $this->history->config('min_fuel_thefts');
    ...
    public function proccess(&$position)
    {
        ...
        foreach ($peaks as $sensorId => $peak) {
            ...
            if ($peak['type'] !== AppendFuelPeaks::STATE_DEC) {
                continue;
            }

            $sensor = $this->sensors[$sensorId];
            $diff = abs($peak['peak_value'] - $peak['begin_value']);
...
//            dump($position->fuel_theft, $position->time); // todo: remove
