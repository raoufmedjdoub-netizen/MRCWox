...
class SharingService
{
    ...
    {
        if ($devices instanceof UserDevicesGroupLoader
            || $devices instanceof UserDevicesLoader) {
            $devices = $devices->getQuerySelected()->pluck('id');
        } elseif ( ! ((is_array($devices) || $devices instanceof Collection)))
            $devices = [$devices];
