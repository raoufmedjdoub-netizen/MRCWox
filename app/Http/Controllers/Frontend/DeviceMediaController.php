...
class DeviceMediaController extends Controller
{
    ...
    private function getDeviceImages(int $device_id, bool $categoriesEnabled, array $filters, $sort)
    {
        ...
        try {
            ...
            $images = new LengthAwarePaginator([], 0, $limit);
        }
    ...
    {
        if (isset($image)) {
            $closestPositions = collect();
            $closestPositions->add($device->positions()
                ->where('valid', true)
                ->whereNotNull('time')
                ->where('time', '<=', $image->created_at)
                ->orderBy('time', 'DESC')
                ->orderBy('id', 'DESC')
                ->first()
            );
            ...
            $closest_position = $closestPositions
                ->filter(function ($position) {
                    return !empty($position);
                })
                ->sortBy(function ($position) use ($image) {
                    return abs(strtotime($position->time) - strtotime($image->created_at));
                })
                ->first();
        }
